...
public interface ProjectReportMemberRepository extends JpaRepository<ProjectReportMemberEntity, Long> {
        ...
        // Check member's isset by report ID
        @Query(value = "SELECT COUNT(prm.id) FROM project_report_member AS prm "
                        + "WHERE prm.first_report_id = :reportId", nativeQuery = true)
        Integer checkMemberIssetByReportId(@Param("reportId") Long reportId);
        ...
        @Transactional
        @Modifying
        @Query(value = "DELETE FROM project_report_member WHERE first_report_id = :first_report_id", nativeQuery = true)
        int deleteMemberByReportId(@Param("first_report_id") Long first_report_id);
